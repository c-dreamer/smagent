...
python3 pipeline/orchestrator.py \
  ...
  --comfy-workflow-template workflows/faith_nexus_image.json
```
...
then set `COMFYUI_URL`, for example `http://127.0.0.1:8188`.
...
fidelity, word timing, one visual per beat, captions, and audio duration. A
